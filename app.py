...
# 3. FULL 2026 FLEET DATABASE (Verified Complete)
airports = {
    # --- CITYFLYER (CF) ---
...
search_iata = st.sidebar.text_input("Enter IATA Code (e.g. STN)", "").upper()
...
for iata, data in weather_data.items():
    ...
    if info['fleet'] in fleet_filter:
        ...
        elif xw > 18: alert_type = "amber"; short_reason = "MARGINAL X-WIND"
...
with m_col:
    ...
    st_folium(m, width=1000, height=700, key="occ_v6")
...
with a_col:
    ...
    if st.session_state.investigate_iata in active_alerts:
        ...
        <div class="reason-box">
            ...
            <p><b>Issue:</b> {d['reason']} detected. Current Visibility is {d['vis']}m and Ceiling is {d['ceiling']}ft.</p>
            <p><b>Impact:</b> This forecast is currently below operating limits and <b>may cause diversions or ATC slots</b> due to the weather. Expect <b>long delays to the operation</b> if conditions persist as forecast in the TAF.</p>
            <hr>
            <small><b>Full TAF Outlook:</b><br>{d['taf']}</small>
        </div>
        """, unsafe_allow_html=True)

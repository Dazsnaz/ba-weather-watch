...
# 2. HUD STYLING (v26.2 CSS RESTORATION)
st.markdown("""
    <style>
...
st_folium(m, width=1200, height=1200, key="map_stable_v292")
...
st.text_area("Handover Report:", value=h_txt, height=200, key="handover_v292", label_visibility="collapsed")import streamlit as st
import folium
from streamlit_folium import st_folium
from avwx import Metar, Taf
import math
...
# 2. HUD STYLING (V29.2 BASE + NAMESPACED v31.0 FIX)
st.markdown("""
    <style>
    /* GLOBAL & MAIN THEME */
    .main { background-color: #001a33 !important; }
    ...
    /* STRATEGY BRIEF (NAMESPACED TO REASON-BOX ONLY) */
    .reason-box { 
        background-color: #ffffff !important; 
    ...
    /* Force Navy strictly inside Reason Box */
    .reason-box h3, .reason-box p, .reason-box b, .reason-box span, 
    .reason-box div, .reason-box th, .reason-box td { 
        color: #002366 !important; 
    }
    /* Force Radio Toggle text inside Brief to Navy (Targeting by hierarchy) */
    .reason-box [data-testid="stWidgetLabel"] p, .reason-box .stRadio label { 
        color: #002366 !important; 
    }
    ...
    .wx-box {
        ...
        color: #002366 !important; white-space: pre-wrap;
    }
...
base_airports = {
    ...
    "AGA": {"icao": "GMAD", "lat": 30.325, "lon": -9.413, "rwy": 90, "fleet": "Euroflyer", "spec": False},
    "SSH": {"icao": "HESH", "lat": 27.977, "lon": 34.394, "rwy": 40, "fleet": "Euroflyer", "spec": False},
    "PFO": {"icao": "LCPH", "lat": 34.718, " Scalable Map Marker": 1, "lon": 32.486, "rwy": 290, "fleet": "Euroflyer", "spec": False},
    "LCA": {"icao": "LCLK", "lat": 34.875, "lon": 33.625, "rwy": 220, "fleet": "Euroflyer", "spec": False},
    "FUE": {"icao": "GCLP", "lat": 28.452, "lon": -13.864, "rwy": 10, "fleet": "Euroflyer", "spec": False},
    "TFS": {"icao": "GCTS", "lat": 28.044, "lon": -16.572, "rwy": 70, "fleet": "Euroflyer", "spec": False},
    "ACE": {"icao": "GCRR", "lat": 28.945, "lon": -13.605, "rwy": 30, "fleet": "Euroflyer", "spec": False},
...
for iata, info in base_airports.items():
    ...
    if info.get('icao') in ["LIRP","LIPE","EDDM","LPPS"]: continue
...
# 10. UI RENDER
st.markdown(f'<div class="ba-header"><div>OCC HUD v31.0</div><div>{datetime.now().strftime("%H:%M")} UTC</div></div>', unsafe_allow_html=True)
m = folium.Map(location=[50.0, 10.0], zoom_start=4, tiles=("CartoDB dark_matter" if map_theme == "Dark Mode" else "CartoDB positron"), scrollWheelZoom=False)
for mkr in map_markers:
    folium.CircleMarker(location=[mkr['lat'], mkr['lon']], radius=7, color=mkr['color'], fill=True, popup=folium.Popup(mkr['content'], max_width=650, auto_pan=True, auto_pan_padding=(150, 150)), tooltip=folium.Tooltip(mkr['content'], direction='top', sticky=False)).add_to(m)
st_folium(m, width=1200, height=1200, key="map_restored_v310")
...
st.text_area("Log:", value=h_txt, height=200, key="log_v310", label_visibility="collapsed")

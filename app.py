...
if st.session_state.investigate_iata in active_alerts:
    ...
    trend = "➡️ Stable"
    if "BECMG" in d['taf'] or "TEMPO" in d['taf']: trend = "📈 Improving/Variable"
    if "FG" in d['metar'] or "DZ" in d['metar']: trend = "📉 Deteriorating"

    st.markdown(f"""
    <div class="reason-box">
        <h3>{st.session_state.investigate_iata} Analysis | Trend: {trend}</h3>
        <p><b>Impact Statement:</b> High risk of holding/diversions. Forecast is below operating limits.</p>

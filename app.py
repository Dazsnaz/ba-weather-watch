...
# 8. UI RENDER
if red_list: st.markdown(f'<div class="marquee"><span>🚨 RED ALERT: {", ".join(red_list)} BELOW MINIMA</span></div>', unsafe_allow_html=True)
...
for mkr in map_markers:
    popup_html = f"""
    <div style="width: 450px; color: black !important; font-family: 'Courier New', monospace;">
        <h4 style="margin:0; color:#002366; border-bottom:1px solid #002366;">{mkr['iata']} Weather Data</h4>
        <div style="display:flex; gap:10px; margin-top:10px;">
            <div style="flex:1; background:#f0f2f6; padding:10px; border-radius:5px;"><b>METAR</b><br><small>{mkr['metar']}</small></div>
            <div style="flex:1; background:#f0f2f6; padding:10px; border-radius:5px;"><b>TAF</b><br><small>{mkr['taf']}</small></div>
        </div>
    </div>
    """
    folium.CircleMarker(location=[mkr['lat'], mkr['lon']], radius=7, color=mkr['color'], fill=True, fill_opacity=0.9, popup=folium.Popup(popup_html, max_width=500)).add_to(m)
...
if st.session_state.investigate_iata in active_alerts:
    ...
    <div class="reason-box">
        ...
        <p><b>Impact Statement:</b> This forecast is currently below operating limits. Expect long delays to the operation if conditions persist.</p>
        <hr>
